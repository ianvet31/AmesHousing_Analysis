...
```{r}
transformed_model = lm(SalePrice ~ MS.Zoning + log(Lot.Area) + Street + Lot.Shape + Land.Slope + House.Style + I(Year.Built^2) + Foundation + Location + Central.Air + Gr.Liv.Area + Bedroom.AbvGr + Kitchen.AbvGr + Kitchen.Qual + TotRms.AbvGrd + (Fireplaces) + I(Garage.Area^2) + (Wood.Deck.SF) + I(Pool.Area^2) + sqrt(Misc.Val) + Exter.Qual + log(TotRms.AbvGrd) + I(BsmtFin.SF.1^3) + Overall.Qual, data = housing_data)

get_loocv_rmse(saleprice_selected_reduced_backward_aic)
get_loocv_rmse(transformed_model)

sqrt(mean((housing_data$SalePrice - fitted(saleprice_selected_reduced_backward_aic)) ^ 2))
sqrt(mean((housing_data$SalePrice - fitted(transformed_model)) ^ 2))
```
