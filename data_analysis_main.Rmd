...
This data set was acquired from Kraggle. It has 79 predictors describing 2,930 houses that were sold in Ames, Iowa between 2006-2010. The `SalePrice` column is the amount the house sold for and will be our response variable.
...
Based on the anova F-tests, the bic forward model is preferred.

In order to find even better model, the several trail and error methods were performed. Some variable were removed and added manually and tested each model by comparison and checking the adjusted r squared values and other metrics. One of the model from such model building process is as below:
```{r}
...
With the help of the plot, trying all possible transformation of the variable and testing how it performs in terms of adjusted r squared and anova test. Two models are selected which are as follow:
...
Now, analyzing the models that seems best fit, with the Fitted Vs. Residual plot as well as QQ plot. 
```{r}
par(mfrow = c(2,2))
...
After adding some polynomial and logarithmic kernel transformations to applicable predictor variables, we can see some noticeable improvement in both RMSE and visual metrics when comparing to the reduced backward-AIC model from earlier.

Now trying some more transformations to the forward BIC model since, it has very good Fitted VS. Residual and QQ Plot.
...
```
Now, seeing the result of Prediction VS. Actual plot for the two good performing models:
